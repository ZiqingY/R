...
#                                                                   #
# install.packages(c("NbClust", "flexclust", "ggdendro","rattle",   #
# "cluster","clusterability", "factoextra","colorhcplot",           #
# "ggplot2"))                                                       # 
#-------------------------------------------------------------------#
...
table(clusters)   # how many values in cluster 1~5

nutrient.scaled$clusters <- clusters    # add a column for clusters
profiles <- nutrient.scaled %>%         # descriptive data for each cluster
  group_by(clusters) %>%
...
library(colorhcplot)
par(mfrow=c(1, 1))      # set the plotting area into 1*1
cl <-factor(clusters, levels=c(1:5), 
            labels=paste("cluster", 1:5))
colorhcplot(fit.average, cl, hang=-1, lab.cex=0.8, lwd=2,
            main="Average-Linkage Clustering\n5 Cluster Solution")    # plotting clusters with color and other trait settings
